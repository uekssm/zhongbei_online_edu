 ...
 <body>
  ...
  <div id="content" class="hide">
    ...
    <div id="contentForBindMenu" class="hide">
      <form id="J_Form_Bind_Menu" class="form-horizontal" action="<{:U('User/add')}>">
        <input type="hidden" name="id">
 		<div class="row">
       		<div class="control-group span8">
	        	<label class="control-label"><s>*</s>权限</label>
  ...
  BUI.use('common/search',function (Search) {
      ...
      columns = [
          ...
          {title:'操作',dataIndex:'',width:200,renderer : function(value,obj){
            if(obj.name=='超级管理员'&&"<{$Think.session.user.username}>"!="admin"){
        		  return '';
              }else{
            	  editStr1 = '<span class="grid-command btn-edit " title="编辑">编辑</span>',
            	  grantStr = '<span class="grid-command btn-grant " title="编辑" onclick="grant_menu(\''+obj.id+'\')">绑定菜单</span>',
...
<script type="text/javascript">
  function grant_menu(id){
    ...
    $.ajax({
      url:'{:U("admin/SysRole/checkbindmenu")}?roleid='+id,
      success:function(msg){
        ...
        $('#xtrees2').val(val.substr(1));
      }
    })
    //bui-tree-item-checked
   // $('.bui-tree-item').addClass('bui-tree-item-checked')
    //console.log($('.bui-tree-item'))
  }
</script>
<body>
</html>
